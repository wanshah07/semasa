    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        <label className="flex flex-1 items-center gap-2 rounded-pill border border-line bg-surface px-4 py-2 shadow-card">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t("Cari tajuk, sumber, ringkasan…", "Search titles, sources, summaries…")}
            className="w-full bg-transparent text-sm outline-none placeholder:text-muted" />
        </label>
        <div className="flex rounded-pill bg-surface-2 p-1 text-xs">
          {[["", t("Semua", "All")], ["ms", "BM"], ["en", "EN"]].map(([v, l]) => (
      ...
      <div className="flex flex-wrap gap-1.5">
        <button onClick={() => setCategory("")}
          ...
          {t("Semua", "All")} {counts.all ? `· ${counts.all}` : ""}
        </button>
